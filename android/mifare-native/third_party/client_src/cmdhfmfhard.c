...
#include "fileutils.h"
#include "mfc_proto.h" // redmi-iceman: mfc_nested_start/capture_one/stop, see acquire_nonces() below
...
// ---------------------------------------------------------------------------
// ANDROID/THN31 PATCH (redmi-iceman project)
//
// Upstream acquire_nonces() asks a USB-connected Proxmark3 to run
// MifareAcquireEncryptedNonces() on its own firmware (armsrc/mifarecmd.c)
// and streams batches of nonce pairs back via SendCommandNG/
// WaitForResponseTimeout(CMD_HF_MIFARE_ACQ_ENCRYPTED_NONCES). We have no
// such device: the phone's own THN31 controller *is* the reader, reached
// through the TMS "Passthrough Mode" (PTM) Binder channel. mfc_nested_*()
// (mfc_proto.c/h, this project) reimplements the same nested-auth capture
// MifareAcquireEncryptedNonces() does, one nonce at a time instead of in
// firmware-side batches, driving it directly through PTM.
//
// Everything below this comment that is NOT inside the acquire_nonces()
// body is untouched upstream logic: add_nonce(), update_nonce_data(),
// shrink_key_space(), apply_sum_a0(), the sums[]/first_byte_num/
// hardnested_stage state machine, hardnested_print_progress(). Only the
// transport that feeds add_nonce() changed.
// ---------------------------------------------------------------------------
static int acquire_nonces(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType, bool nonce_file_write, bool slow, char *filename) {
    ...
    (void)slow; // upstream's device-side "slow" pacing flag doesn't apply to our transport
    ...
    uint32_t got_cuid = 0;
    if (mfc_nested_start(blockNo, keyType, key, &got_cuid) != PM3_SUCCESS) {
        return PM3_ESOFT;
    }
    cuid = got_cuid;
    ...
    do {
        uint32_t nt_enc = 0;
        uint8_t par_enc = 0; // 4-bit encrypted parity, same convention as add_nonce()'s 2nd arg upstream

        if (mfc_nested_capture_one(trgBlockNo, trgKeyType, &nt_enc, &par_enc) != PM3_SUCCESS) {
            if (nonce_file_write) fclose(fnonces);
            mfc_nested_stop();
            return PM3_ESOFT;
        }
        ...
        if (nonce_file_write) {
            // Note: upstream's MFC_NONCE_PAIR_SIZE-byte record packs two
            // nonces per write since the firmware samples them in pairs;
            // we capture one at a time, so we pad the second slot with
            // zero. A file written here is NOT byte-compatible with
            // upstream's own nonce dumps -- fine for our own re-use
            // in-process, but don't feed it to desktop Iceman's `-r`.
            uint8_t pair_buf[MFC_NONCE_PAIR_SIZE];
            memset(pair_buf, 0, sizeof(pair_buf));
            num_to_bytes(nt_enc, 4, pair_buf);
            pair_buf[8] = (uint8_t)(par_enc << 4);
            fwrite(pair_buf, 1, sizeof(pair_buf), fnonces);
            fflush(fnonces);
        }
        ...
        } else if ((num_acquired_nonces & 0x1F) == 0) {
            // Throttled vs. upstream (which recalculates once per ~60-nonce
            // USB batch): we capture one nonce at a time, so recomputing
            // update_nonce_data()/shrink_key_space() after every single one
            // would pay their O(n) cost far more often than upstream does.
            update_nonce_data(true);
            acquisition_completed = shrink_key_space(&brute_force_depth);
            hardnested_print_progress(num_acquired_nonces, "Apply bit flip properties", brute_force_depth, 0);
        }
    ...
    } while (acquisition_completed == false);
